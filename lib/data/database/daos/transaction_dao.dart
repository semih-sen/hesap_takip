...
class TransactionListRowData {
  ...
  /// ARGB int color of the [Accounts] row owning this transaction's wallet
  /// (`Wallet.accountId → Account.colorValue`). Drives the row's left stripe.
  /// Independent of [categories]'s accent color, which drives the tint.
  ...
  /// True for every transaction row created as one leg of a transfer, including
  /// cross-account legs that are stored as income/expense for summary purposes.
  final bool isTransferGenerated;

  /// True when generated by a recurring rule (Phase 10).
  final bool isRecurring;

  /// The transaction's amount snapshot in the app's base currency.
...
@DriftAccessor(
  tables: [Transactions, TransactionCategories, Categories, Wallets, Accounts],
)
class TransactionDao extends DatabaseAccessor<AppDatabase>
  ...
  /// Detaches a single generated transaction from its recurring rule (series
  /// edit "this occurrence only"): clears `recurringRuleId` so it becomes an
  /// ordinary one-off.
  Future<void> detachFromRecurringRule(int transactionId) =>
      (update(transactions)..where((t) => t.id.equals(transactionId))).write(
        const TransactionsCompanion(recurringRuleId: Value<int?>(null)),
      );

  /// Detaches EVERY transaction generated by [ruleId] (clears their
  /// `recurringRuleId`). Used before hard-deleting a rule so the FK reference is
  /// released and the generated rows survive as plain transactions.
  Future<int> detachAllFromRecurringRule(int ruleId) =>
      (update(transactions)..where((t) => t.recurringRuleId.equals(ruleId)))
          .write(const TransactionsCompanion(recurringRuleId: Value<int?>(null)));
  ...
  Stream<List<TransactionListRowData>> watchTransactionRows(
    ...
    return customSelect(
      ...
      't.base_amount_minor AS base_amount_minor, '
      'w.name AS wallet_name, '
      'acc.color_value AS account_color_value, '
  ...
  TransactionListRowData _mapListRow(QueryRow row) {
    ...
    return TransactionListRowData(
      ...
      isTransferGenerated: row.read<String?>('transfer_group_id') != null,
      accountColorValue: row.read<int>('account_color_value'),
      baseAmountMinor: row.read<int>('base_amount_minor'),
    );
  }
