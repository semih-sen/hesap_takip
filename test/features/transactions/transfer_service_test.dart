...
void main() {
  ...
  test(
    'same-currency transfer moves both balances; delete removes both legs',
    () async {
  ...
  test(
    'cross-currency transfer: converted destination + own base snapshots',
    () async {
      final int account = await seedAccount();
  ...
  test('undo keeps both legs; flush commits both', () async {
    ...
    final List<PendingEntry> entries = container.read(
      pendingActionQueueProvider,
    );
    expect(
      entries.isPendingDeleted(
        EntityRef(UndoEntityType.transaction, legIds[0]),
      ),
      isTrue,
    );
    expect(
      entries.isPendingDeleted(
        EntityRef(UndoEntityType.transaction, legIds[1]),
      ),
      isTrue,
    );
  ...
  group('auto-append transfer details to notes', () {
    ...
    test(
      'cross-account transfer: note includes account/wallet names',
      () async {
    ...
    test(
      'user-provided note: transfer detail appended after newline',
      () async {
        final int account = await seedAccount();
        final int w1 = await seedWallet(account);
        final int w2 = await seedWallet(account);
